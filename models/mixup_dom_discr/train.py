...
from models.zoo.domain_discriminators import DAFL_FAS_mixup
from training.config_parser import read_config
from training.metric import AverageMeter, Tracker
from training.experiment import Experiment
from training.mixup import get_perm, mixup_criterion
    ...
    for epoch in range(0, args.epochs):
        ...
        for (data, label, domain_label, multi_label) in tqdm(train_loader, dynamic_ncols=True, desc=f"epoch {epoch} train"):
            ...
            if torch.sum(live_idx * 1) > 0:
                ...
                live_embd = embd[[live_idx]]
                live_dom_label = domain_label[[live_idx]]
                live_index = get_perm(live_embd, use_cuda=True)
                live_embd1 = live_embd[live_index, :]
                live_dom_label1 = live_dom_label[live_index]
                live_dis_logits = model.forward_mix_live(live_embd, live_embd1, lam)
                live_dis_loss = mixup_criterion(loss_func_ccdd, live_dis_logits, live_dom_label, live_dom_label1, lam)
            
            if torch.sum(spoof_idx * 1) > 0:
                spoof_loss = loss_func_cls(logits_cls[[spoof_idx]], label[[spoof_idx]])
                spoof_embd = embd[[spoof_idx]]
                spoof_dom_label = domain_label[[spoof_idx]]
                spoof_index = get_perm(spoof_embd, use_cuda=True)
                spoof_embd1 = spoof_embd[spoof_index, :]
                spoof_dom_label1 = spoof_dom_label[spoof_index]
                spoof_dis_logits = model.forward_mix_spoof(spoof_embd, spoof_embd1, lam)
                spoof_dis_loss = mixup_criterion(loss_func_ccdd, spoof_dis_logits, spoof_dom_label, spoof_dom_label1, lam)
