...
def train(args: Namespace):
    ...
    for epoch in range(0, args.epochs):
        ...
        for (data, label, domain_label, multi_label) in tqdm(train_loader, dynamic_ncols=True, desc=f"epoch {epoch} train"):
            ...
            logits_cls, logits_dis = model(data)
            ...
            dis_loss = loss_func_ccdd(logits_dis, domain_label)
            ...
            train_dis_loss.append(0. if dis_loss == 0. else dis_loss.detach().item())
            
            loss = loss_func_cls(logits_cls, label) + args.ccdd_lambda * dis_loss
            loss.backward()
        ...
        train_losses = {
            ...
            'train_dis_loss': train_dis_loss.value,
        }
